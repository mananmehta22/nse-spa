...
const Jan_15 = () => {
  ...
  const loadData = async () => {
    const response = await axios.get("http://localhost:5000/jan_15");
          ...
          {data.map((item, index) => {
            return (
              <tr key={item.sr_no}>
                ...
                <td>
                  <Link to={`/update_15/${item.sr_no}`}>
                  <button className='btn btn-edit'>Edit</button>
                  </Link>
                  <Link to={`/view_15/${item.sr_no}`}>
